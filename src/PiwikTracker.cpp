...
bool CPiwikTracker::setCustomVariable(
    const int id,
...
int CPiwikTracker::doTrackEvent(
...
int CPiwikTracker::doTrackContentImpression(
    const wchar_t *const contentName,
    const wchar_t *const contentPiece,
...
int CPiwikTracker::doTrackContentInteraction(
    const wchar_t *const interaction,
    const wchar_t *const contentName,
    const wchar_t *const contentPiece,
    const wchar_t *const contentTarget
) {
...
int CPiwikTracker::doTrackSiteSearch(
    const wchar_t *const keyword,
    const wchar_t *const category,
    const int countResults
) {
...
bool CPiwikTracker::addEcommerceItem(
...
int CPiwikTracker::doTrackEcommerceOrder(
    const wchar_t *const orderId,
    const float grandTotal,
    const float subTotal,
    const float tax,
    const float shipping,
...
bool CPiwikTracker::setEcommerceView(
    const char *const sku,
    const char *const name,
...
char *CPiwikTracker::getUrlTrackEvent(
    const wchar_t *const category,
    const wchar_t *const action,
    const wchar_t *const name,
    const char *const value
...
char *CPiwikTracker::getUrlTrackContentImpression(
    const wchar_t *const contentName,
    const wchar_t *const contentPiece,
    const wchar_t *const contentTarget
) {
...
char *CPiwikTracker::getUrlTrackContentInteraction(
    const wchar_t *const interaction,
    const wchar_t *const contentName,
    const wchar_t *const contentPiece,
    const wchar_t *const contentTarget
...
char *CPiwikTracker::getUrlTrackSiteSearch(
    const wchar_t *const keyword,
    const wchar_t *const category,
    const int countResults
) {
...
char *CPiwikTracker::getUrlTrackEcommerceOrder(
    const wchar_t *const orderId,
    const float grandTotal,
    const float subTotal,
    const float tax,
...
std::string CPiwikTracker::getRequest(unsigned int idSite) {
    ...
    url += (localHour != 0xFF && localMinute != 0xFF && localSecond != 0xFF)
               ? "&h=" + IntToStr(localHour) + "&m=" + IntToStr(localMinute) + "&s=" + IntToStr(localSecond)
               : "";
    ...
    url +=
        (((pageCharset.size() > 0) && (pageCharset != DEFAULT_CHARSET_PARAMETER_VALUES)) ? "&cs=" + pageCharset : "");
...
bool CPiwikTracker::loadVisitorIdCookie() {
    ...
    bool res;
    cookieVisitorId =
        parts[0]; // provides backward compatibility since getVisitorId() didn't change any existing VisitorId value
...
void CPiwikTracker::setPlugins(
    const bool flash,
    const bool java,
    const bool director,
    const bool quickTime,
    const bool realPlayer,
    const bool pdf,
    const bool windowsMedia,
    const bool gears,
    const bool silverlight
) {
    plugins = "&fla=" + IntToStr(flash ? 1 : 0) + "&java=" + IntToStr(java ? 1 : 0) +
              "&dir=" + IntToStr(director ? 1 : 0) + "&qt=" + IntToStr(quickTime ? 1 : 0) +
              "&realp=" + IntToStr(realPlayer ? 1 : 0) + "&pdf=" + IntToStr(pdf ? 1 : 0) +
              "&wma=" + IntToStr(windowsMedia ? 1 : 0) + "&gears=" + IntToStr(gears ? 1 : 0) +
